# Provides basic Unit Tests for the major functionality in the payment_provider (payment.provider) entity definition

import unittest
from unittest.mock import patch, MagicMock
from odoo.tests import TransactionCase, tagged
from odoo.exceptions import UserError, ValidationError
from odoo import fields
from datetime import datetime, timedelta
import json
import logging

_logger = logging.getLogger(__name__)


@tagged('post_install', '-at_install')
class TestPaymentProviderMonero(TransactionCase):
    """
    Unit test class for Monero payment provider logic.
    Covers setup, wallet access, address generation, rate fetching,
    payment creation, and inverse/computed fields.
    """

    def setUp(self):
        """
        Prepare test environment including a Monero payment provider,
        currencies, a sample order and related entities.
        """
        super(TestPaymentProviderMonero, self).setUp()
        self.PaymentProvider = self.env['payment.provider']
        self.Currency = self.env['res.currency']
        self.MoneroPayment = self.env['monero.payment']

        self.currency_usd = self.env.ref('base.USD')
        self.currency_eur = self.env.ref('base.EUR')

        self.provider = self.PaymentProvider.create({
            'name': 'Monero Test Provider',
            'code': 'monero_rpc',
            'state': 'enabled',
            'rpc_url': 'http://localhost:38082/json_rpc',
            'rpc_user': 'testuser',
            'rpc_password': 'testpass',
            'wallet_file': 'testwallet',
            'wallet_password': 'walletpass',
            'wallet_dir': '/tmp',
            'network_type': 'mainnet',
            'daemon_rpc_url': 'http://localhost:38081/json_rpc',
            'confirmation_threshold': 10,
            'manual_rates': json.dumps({'USD': 200, 'EUR': 180}),
            'use_subaddresses': True,
        })

        self.test_order = self.env['sale.order'].create({
            'partner_id': self.env['res.partner'].create({'name': 'Test Partner'}).id,
            'order_line': [(0, 0, {
                'product_id': self.env['product.product'].create({
                    'name': 'Test Product',
                    'list_price': 100.0
                }).id,
                'product_uom_qty': 1,
                'price_unit': 100.0,
            })]
        })

        # Issue 13: monero.transaction.confirmations is store=False and reads
        # current_height from monero.daemon. Without a daemon record, all
        # confirmations compute to 0 and confirmed state is never reachable in tests.
        self.daemon_record = self.env['monero.daemon'].create({
            'state': 'online',
            'network': 'mainnet',
            'current_height': 2000,
            'target_height': 2000,
            'last_checked': fields.Datetime.now(),
        })

    def test_create_monero_provider(self):
        """Test creation of Monero provider with expected properties."""
        self.assertEqual(self.provider.code, 'monero_rpc')
        self.assertEqual(self.provider.network_type, 'mainnet')
        self.assertEqual(self.provider.confirmation_threshold, 10)
        self.assertTrue(self.provider.use_subaddresses)

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.Wallet')
    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.JSONRPCWallet')
    def test_get_wallet_client_success(self, mock_jsonrpc, mock_wallet):
        """Ensure wallet client initializes correctly."""
        # Issue 97: os.path.exists check was removed from _get_wallet_client
        # so no patch needed; mock Wallet/JSONRPCWallet directly.
        mock_jsonrpc.return_value = MagicMock()
        mock_wallet.return_value = MagicMock()
        wallet = self.provider._get_wallet_client()
        self.assertIsNotNone(wallet)

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.Wallet')
    def test_get_wallet_client_failure(self, mock_wallet):
        """Test wallet connection failure raises UserError."""
        mock_wallet.side_effect = Exception("Connection failed")
        with self.assertRaises(UserError):
            self.provider._get_wallet_client()

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.JSONRPCDaemon')
    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.Daemon')
    def test_get_daemon_client_success(self, mock_daemon, mock_jsonrpc):
        """Ensure daemon initializes without error."""
        mock_jsonrpc.return_value = MagicMock()
        mock_daemon.return_value = MagicMock()
        daemon = self.provider._get_daemon()
        self.assertIsNotNone(daemon)

    def test_validate_address(self):
        """Check that address validation rejects invalid input."""
        # A real Monero address is 95 characters long — 'testaddress' should fail
        self.assertFalse(self.provider._validate_address('testaddress'))
        self.assertFalse(self.provider._validate_address('testaddress', is_subaddress=True))
        self.assertFalse(self.provider._validate_address(''))

    @patch('requests.get')
    def test_cron_update_xmr_rates_api_success(self, mock_get):
        """Verify that exchange rates update from API."""
        mock_response = MagicMock()
        mock_response.json.return_value = {'monero': {'usd': 200.0}}
        mock_get.return_value = mock_response
        rate = self.provider._cron_update_xmr_rates('USD')
        self.assertEqual(rate, 200.0)

    @patch('requests.get')
    def test_cron_update_xmr_rates_api_failure(self, mock_get):
        """Fallback to manual rate on API error."""
        mock_get.side_effect = Exception("API error")
        rate = self.provider._cron_update_xmr_rates('USD')
        self.assertEqual(rate, 200.0)

    def test_get_manual_rate(self):
        """Check manual rate fallback works for configured and unknown currencies."""
        self.assertEqual(self.provider.get_manual_rate('USD'), 200.0)
        self.assertEqual(self.provider.get_manual_rate('EUR'), 180.0)
        self.assertIsNone(self.provider.get_manual_rate('JPY'))

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_fetch_wallet_addresses_success(self, mock_wallet):
        """Ensure fetching wallet address populates correct values."""
        mock_account = MagicMock()
        mock_account.primary_address.return_value = MagicMock(address='addr1')
        mock_account.balance.return_value = MagicMock(balance=1000000000000, unlocked_balance=800000000000)
        mock_account.index = 0
        mock_account.label = "Test Account"

        mock_wallet.return_value.accounts.return_value = [mock_account]

        result = self.provider.fetch_wallet_addresses()
        self.assertTrue(result)
        self.assertEqual(self.provider.wallet_status, 'ready')
        self.assertEqual(self.provider.account_balance, 1.0)
        self.assertEqual(self.provider.unlocked_balance, 0.8)

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_fetch_wallet_addresses_failure(self, mock_wallet):
        """Fetch failure should raise error and update status."""
        mock_wallet.side_effect = Exception("Wallet error")
        with self.assertRaises(UserError):
            self.provider.fetch_wallet_addresses()
        self.assertEqual(self.provider.wallet_status, 'error')

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_create_monero_from_fiat_payment_subaddress(self, mock_wallet):
        """Test creating payment using subaddress mode — payment_id must be a str."""
        mock_wallet.return_value.new_address.return_value = ('subaddr1', 42)
        payment = self.provider._create_monero_from_fiat_payment(
            order_ref='TEST123',
            amount=100.0,
            currency=self.currency_usd,
            order_id=self.test_order
        )
        self.assertEqual(payment.address_seller, 'subaddr1')
        self.assertTrue(payment.is_subaddress)
        # payment_id must be a string (Char field), not an integer
        self.assertIsInstance(payment.payment_id, str)
        self.assertEqual(payment.payment_id, '42')

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_create_monero_from_fiat_payment_integrated(self, mock_wallet):
        """Create payment using integrated address fallback."""
        self.provider.use_subaddresses = False
        self.provider.wallet_address_value = 'baseaddr1'
        mock_wallet.return_value.integrated_address.return_value = 'integrated1'

        payment = self.provider._create_monero_from_fiat_payment(
            order_ref='TEST123',
            amount=100.0,
            currency=self.currency_usd,
            order_id=self.test_order
        )
        self.assertEqual(payment.address_seller, 'integrated1')
        self.assertFalse(payment.is_subaddress)

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_create_monero_from_fiat_payment_no_base_address(self, mock_wallet):
        """Integrated payment should fail with no base address."""
        self.provider.use_subaddresses = False
        self.provider.wallet_address_value = False
        with self.assertRaises(UserError):
            self.provider._create_monero_from_fiat_payment(
                order_ref='TEST123',
                amount=100.0,
                currency=self.currency_usd,
                order_id=self.test_order
            )

    def test_compute_wallet_selection(self):
        """Test computation of wallet address dropdown entries."""
        self.provider.wallet_addresses = [
            {'address': 'addr1', 'label': 'Account 1'},
            {'address': 'addr2', 'label': 'Account 2'}
        ]
        result = self.provider._compute_wallet_selection()
        self.assertEqual(len(result), 2)

    def test_compute_account_details(self):
        """Ensure account balance and index are computed from cached wallet_addresses (no live RPC)."""
        self.provider.wallet_addresses = [{
            'address': 'addr1', 'label': 'Account 1',
            'balance': 1.5, 'unlocked_balance': 1.2, 'account_index': 0
        }]
        self.provider.wallet_address = 'addr1'
        # _compute_account_details reads from cache — no wallet RPC needed
        self.provider._compute_account_details()
        self.assertEqual(self.provider.account_balance, 1.5)

    def test_inverse_wallet_address(self):
        """Ensure inverse method updates value field."""
        self.provider.wallet_addresses = [{
            'address': 'addr1', 'label': 'Account 1',
            'balance': 1.5, 'unlocked_balance': 1.2, 'account_index': 0
        }]
        self.provider.wallet_address = 'addr1'
        self.provider._inverse_wallet_address()
        self.assertEqual(self.provider.wallet_address_value, 'addr1')

    def test_onchange_wallet_address(self):
        """Test onchange properly triggers updates."""
        self.provider.wallet_addresses = [{
            'address': 'addr1', 'label': 'Account 1',
            'balance': 1.5, 'unlocked_balance': 1.2, 'account_index': 0
        }]
        self.provider.wallet_address = 'addr1'
        self.provider._onchange_wallet_address()
        self.assertEqual(self.provider.account_balance, 1.5)

    def test_setup_monero_payment_method(self):
        """Check payment method auto-setup creates Monero RPC method."""
        method = self.provider._setup_monero_payment_method()
        self.assertEqual(method.code, 'monero_rpc')

    def test_get_compatible_payment_methods(self):
        """Test provider returns self as compatible for matching partner + currency."""
        result = self.provider._get_compatible_payment_methods(
            partner_id=self.test_order.partner_id.id,
            currency_id=self.currency_usd.id
        )
        self.assertEqual(result.code, 'monero_rpc')

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_generate_subaddress(self, mock_wallet):
        """Subaddress creation using wallet interface."""
        mock_wallet.return_value.new_address.return_value = ('subaddr1', 'index1')
        result = self.provider._generate_subaddress(label='Test')
        self.assertEqual(result['address'], 'subaddr1')

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_generate_integrated_address(self, mock_wallet):
        """Test generating integrated address via wallet client."""
        mock_wallet.return_value.integrated_address.return_value = 'integrated1'
        result = self.provider._generate_integrated_address('baseaddr1', 'paymentid1')
        self.assertEqual(result, 'integrated1')

    def test_cron_update_payment_provider(self):
        """Ensure cron task updates wallet addresses."""
        with patch.object(self.provider, 'fetch_wallet_addresses') as mock_fetch:
            self.provider._cron_update_payment_provider()
            mock_fetch.assert_called_once()

    def test_create_monero_from_fiat_payment_public(self):
        """Public wrapper test for creating payment from fiat input."""
        with patch.object(
            self.provider, '_create_monero_from_fiat_payment'
        ) as mock_create:
            mock_create.return_value = MagicMock(
                payment_id='test123',
                address_seller='addr1',
                is_subaddress=True,
                image_qr='qrdata',
                amount=0.5,
                exchange_rate=200.0,
                order_ref='TEST123',
                state='pending',
                expiration=datetime.now(),
                original_amount=100.0,
                original_currency='USD'
            )

            result = self.provider.create_monero_from_fiat_payment(
                pos_reference='TEST123',
                amount=100.0,
                currency_name='USD',
                extra_data=None
            )
            self.assertTrue(result['success'])
            self.assertEqual(result['payment_id'], 'test123')

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_check_payment_status_pending(self, mock_wallet_client):
        """check_payment_status returns pending when no transfers received."""
        mock_wallet = MagicMock()
        mock_wallet.incoming.return_value = []
        mock_wallet_client.return_value = mock_wallet

        payment = self.env['monero.payment'].create({
            'amount': 0.5,
            'address_seller': 'testaddr123',
            'order_ref': 'SO-TEST',
            'state': 'pending',
        })

        result = payment.check_payment_status()
        self.assertEqual(result['state'], 'pending')
        self.assertEqual(float(result['amount_received']), 0.0)

    @patch('odoo.addons.payment_monero_rpc.models.payment_provider.PaymentProviderMonero._get_wallet_client')
    def test_check_payment_status_confirmed(self, mock_wallet_client):
        """check_payment_status transitions to confirmed with sufficient confirmations."""
        from decimal import Decimal

        mock_tx = MagicMock()
        mock_tx.hash = 'abc123'
        mock_tx.fee = Decimal('0.0001')
        mock_tx.height = 1000
        mock_tx.timestamp = None
        mock_tx.confirmations = 15
        mock_tx.note = ''
        mock_tx.key = None
        mock_tx.double_spend_seen = False
        mock_tx.in_pool = False
        mock_tx.extra = ''

        mock_transfer = MagicMock()
        mock_transfer.local_address = 'testaddr_confirmed'
        mock_transfer.amount = Decimal('0.5')
        mock_transfer.transaction = mock_tx
        mock_transfer.subaddr_index = MagicMock(minor=1)
        mock_transfer.account_index = 0
        mock_transfer.stealth_address = None

        mock_wallet = MagicMock()
        mock_wallet.incoming.return_value = [mock_transfer]
        mock_wallet_client.return_value = mock_wallet

        payment = self.env['monero.payment'].create({
            'amount': 0.5,
            'address_seller': 'testaddr_confirmed',
            'order_ref': 'SO-CONFIRM',
            'state': 'pending',
        })

        result = payment.check_payment_status()
        self.assertEqual(result['state'], 'confirmed')

    def test_payment_confirmed_posts_message(self):
        """_payment_confirmed writes state and posts chatter message."""
        payment = self.env['monero.payment'].create({
            'amount': 0.5,
            'address_seller': 'testaddr_msg',
            'order_ref': 'SO-MSG',
            'state': 'paid_unconfirmed',
        })
        # Patch email template to avoid missing-template error in test DB
        with patch.object(self.env['mail.template'], 'send_mail', return_value=True):
            with patch.object(payment.env, 'ref', return_value=MagicMock(send_mail=MagicMock())):
                # Issue 25: fixed signature is _payment_confirmed(self, values) —
                # do NOT pass payment as a positional arg (old two-arg signature)
                payment._payment_confirmed({
                    'amount_received': 0.5,
                    'state': 'confirmed',
                    'last_check': fields.Datetime.now(),
                })
        self.assertEqual(payment.state, 'confirmed')

