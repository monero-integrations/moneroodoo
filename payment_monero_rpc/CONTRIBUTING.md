...
- **Security Requirements**  
  ...
  - Safe key management  
  - Multisig and view-only wallet considerations

- **Privacy Preservation**  
  - Avoiding data leaks  
  - Preventing linkage of addresses or transactions unnecessarily

- **Daemon and Wallet Communication**  
  - Interfacing with `monerod`  
  - Managing `monero-wallet-rpc` sessions

> **Note:** The Monero integration must never compromise user privacy or system security.
...
- Separate Monero RPC calls into dedicated service classes to avoid code duplication.

### 2. RPC Usage

- Use **monero-python** when practical.  
- For direct RPC calls, handle all edge cases and RPC errors explicitly.
- Use **subaddresses** or **integrated addresses** for identifying incoming transactions per order or user where applicable.
...
- Define and track states clearly:  
  ...
  - `confirmed`  
  - `failed` or `expired`
- Ensure that transaction polling is **idempotent** to avoid duplicates.
