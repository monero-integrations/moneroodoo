...
class MoneroController(http.Controller):
    ...
    def monero_transaction(self, verify_validity=False, **kwargs):
        ...
        _logger.info(f'setting sales_order state to "sent" for sales_order: {sales_order.id}')
    ...
    def payment_token(self, pm_id=None, **kwargs):
        ...
        if tx_id:
            ...
            transaction = sales_order._create_payment_transaction(tx_val)
            _logger.info(f'created transaction: {transaction.id} for payment token: {token.id}')
            _logger.info(f'token.acquirer_ref = {token.acquirer_ref}')
            if transaction.acquirer_id.is_cryptocurrency:
                _logger.info(f'Processing cryptocurrency payment acquirer: {transaction.acquirer_id.name}')
                _logger.info(f'setting sales_order state to "sent" for sales_order: {sales_order.id}')
                sales_order.sudo().update({'state': 'sent'})
                _logger.info(f'setting transaction state to "pending" for sales_order: {sales_order.id}')
