...
class TestMoneroSalesOrder(TestSaleCommon):
    ...
    def test_sale_order(self):
        ...
        self.assertEqual(self.sale_order.amount_total, 1240.0,
        ...
        self.assertTrue(self.sale_order.state == 'sent',
        ...
        self.assertEqual(len(invoice2.invoice_line_ids), 2,
                         'Sale: second invoice is missing lines')
        ...
        self.assertTrue(self.sale_order.invoice_status == 'upselling',
        ...
        self.assertEqual(len(invoice3.invoice_line_ids), 1,
                         'Sale: third invoice is missing lines')
        self.assertEqual(invoice3.amount_total, 720.0,
                         'Sale: second invoice total amount is wrong')
        self.assertTrue(self.sale_order.invoice_status == 'invoiced',
    ...
    def test_sale_order_send_to_self(self):
        ...
        sale_order.with_context(**email_ctx).with_user(
            self.company_data['default_user_salesman']).message_post_with_template(
            mail_template.id)
        self.assertTrue(sale_order.state == 'sent',
                        'Sale : state should be changed to sent')
        mail_message = sale_order.message_ids[0]
        self.assertEqual(mail_message.author_id, sale_order.partner_id,
    ...
    def test_unlink_cancel(self):
        ...
        self.assertTrue(so_copy.unlink(),
                        'Sale: deleting a quotation should be possible')
        ...
        self.assertTrue(so_copy.state == 'cancel',
                        'Sale: SO should be in state "cancel"')
        with self.assertRaises(AccessError):
            so_copy.with_user(self.company_data['default_user_employee']).unlink()
        self.assertTrue(so_copy.unlink(),
                        'Sale: deleting a cancelled SO should be possible')
        ...
        self.assertTrue(self.sale_order.state == 'done',
                        'Sale: SO should be in state "done"')
        with self.assertRaises(UserError):
            self.sale_order.unlink()
    ...
    def test_cost_invoicing(self):
        ...
        so = self.env['sale.order'].create({
            ...
            'order_line': [(0, 0, {'name': prod_gap.name, 'product_id': prod_gap.id,
                                   'product_uom_qty': 2,
        ...
        inv = self.env['account.move'].with_context(
            default_move_type='in_invoice').create(account_move)
        inv.action_post()
    ...
    def test_so_create_multicompany(self):
        ...
        inv = so_1.sudo() \
            ...
            ._create_invoices()
        self.assertEqual(inv.company_id, self.company_data['company'],
    ...
    def test_multi_currency_discount(self):
        ...
        sales_order = product_1_ctxt.with_context(
            mail_notrack=True,
    ...
    def test_assign_sales_team_from_partner_user(self):
        ...
        self.assertEqual(sale_order.team_id.id, self.crm_team0.id,
    ...
    def test_assign_sales_team_from_partner_team(self):
        ...
        sale_order.onchange_partner_id()
        self.assertEqual(sale_order.team_id.id, self.crm_team1.id,
