...
class TestMoneroPaymentAcquirer(TransactionCase):
    ...
    @patch("odoo.addons.monero-rpc-odoo.models.monero_acq.http_requests.get")
    ...
    def test_update_xmr_rate_updates_existing(self, mock_get):
        ...
        # Call twice — should update, not create a second record
        self.provider.update_xmr_rate()
        self.provider.update_xmr_rate()
    ...
    def test_update_xmr_rate_no_xmr_currency(self, mock_get):
        ...
        # Deactivate XMR currency if it exists
        xmr = self.env["res.currency"].search([("name", "=", "XMR")], limit=1)
        if xmr:
    ...
    @patch("odoo.addons.monero-rpc-odoo.models.monero_acq.JSONRPCWallet")
    def test_post_process_confirms_order(self, mock_rpc):
