    ...
    def process_transaction(self, transaction, token, num_confirmation_required):
        try:
            ...
            raise MoneroPaymentAcquirerRPCUnauthorized(
                "Monero Processing Queue: Monero Payment Acquirer can't authenticate with RPC due to user name or password"
            )
        except MoneroPaymentAcquirerRPCSSLError:
            raise MoneroPaymentAcquirerRPCSSLError(
                "Monero Processing Queue: Monero Payment Acquirer experienced an SSL Error with RPC"
            )
        except Exception as e:
            raise Exception(
                f"Monero Processing Queue: Monero Payment Acquirer experienced an Error with RPC: {e.__class__.__name__}"
            )

        incoming_payment = wallet.incoming(local_address=token.name, unconfirmed=True)

        if incoming_payment == []:
            job = self.env["queue.job"].sudo().search([("uuid", "=", self.env.context.get("job_uuid"))])
            ...
            if job.retry == job.max_retries - 1:
                ...
                log_msg = (
                    ...
                    f"Cancelling order # {self.id}. Action: Nothing"
                )
                ...
                exception_msg = (
                    ...
                    "This is fine. Another job will execute. Action: Nothing"
                )
        ...
        if len(incoming_payment) == 1:
            ...
            conf_err_msg = (
                f"PaymentAcquirer: {transaction.acquirer_id.provider} Subaddress: {token.name} "
                "Status: Waiting for more confirmations Confirmations: current {this_payment.transaction.confirmations}, "
                f"expected {num_confirmation_required} Action: none"
            )
            ...
            if transaction.amount == transaction_amount_rounded:
                ...
                _logger.info(
                    f"Monero payment recorded for sale order: {self.id}, associated with subaddress: {token.name}"
                )
